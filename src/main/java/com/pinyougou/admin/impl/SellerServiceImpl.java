...
public class SellerServiceImpl implements SellerService {
	...
	public Map findByName(String name, String password) {
		Map map =new HashMap();
		TbSeller selller = null;
		try {
				selller = sellerMapper.selectByPrimaryKey(name);
				if(selller==null) {
					map.put("status", false);
					map.put("message", "商家用户名不存在！");
					return map;
				}
				
				if(!password.equals(selller.getPassword())) {
					map.put("status", false);
					map.put("message", "用户名或者密码错误！");
					return map;
				}
				map.put("status", true);
				map.put("message",name+"登录成功！");
				map.put("seller", selller);
				redisTemplate.boundHashOps("seller").put("username", selller);
		}catch(Exception e) {
			System.out.println("系统出错！" +" error: "+e.getMessage());
		}
